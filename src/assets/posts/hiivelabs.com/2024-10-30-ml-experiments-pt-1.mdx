...
author: hiive
description: >-
    This post kicks off a series of experiments by focusing on the first attempt
    to use Large Language Models (LLMs) to
...
thumbnail: >-
    /src/assets/images/hiivelabs.com/blog/2024-10-30-ml-experiments-pt-1/thumbnail.png
categories:
  - gamedev
  - machine-learning
...
Even with a good policy, it's still entirely possible that any reasonably complex NPC's policy will be impractically large. The go-to method for dealing with this is to take advantage of the function approximation capabilities of neural networks and use a technique known as Deep Q-Learning, where the policy, instead of being represented as *n*-dimensional
...
The gridworld for this experiment is a simple 20×20 grid, identical to the image. This shows a 20×20 grid with one <InlineSpan text="green" style="color:limegreen" />square (food), one <InlineSpan text="blue" style="color:cornflowerblue" /> square (water), one <InlineSpan text="red" style="color:red" /> square (refuge) and a black circle (entity) within the refuge.

<LightboxVideo video="/src/assets/images/hiivelabs.com/blog/2024-10-30-ml-experiments-pt-1/gridworld.mp4" inlineImg="/src/assets/images/hiivelabs.com/blog/2024-10-30-ml-experiments-pt-1/gridworld-inline.png" previewImg="/src/assets/images/hiivelabs.com/blog/2024-10-30-ml-experiments-pt-1/gridworld-video-preview.png" alt="Experiment 1 Gridworld" caption="Gridworld in action. Click to play video." />
...
As the entity performs actions, these attributes are increased or decreased depending on the action and its results. After each run (i.e. when it dies) the entity is rewarded based on how long it managed to survive. This reward is then applied to each state the entity found itself in, increasing the likelihood that an action is chosen again in a specific state if the survival time was longer than the expected value (this is the survival time of the entity if it remained in the open taking no actions), or decreasing it if the survival time is less than the expected value. The delta applied to the action-state selection is the difference between the expected value and the actual value, multiplied by <EquationSnippet equation="\alpha" inline />, the learning rate. The learning rate is a small valued constant that ensures updates to the policy are gradual, helping to avoid overshooting optimal values on the policy’s hypersurface.

<ContentWarning warning="CONTENT WARNING: EXCESSIVE MATH — CLICK TO SHOW AT YOUR OWN RISK!">
    Mathematically, this looks like this:

    <EquationSnippet equation="\Delta_R=R_{actual}-R_{expected}" inline={false} />

    <p>
        <div class="equation-note">
            – where <EquationSnippet equation="R_C" inline /> is the given reward value for the given circumstance  <EquationSnippet equation="C" inline /> (<EquationSnippet equation="actual" inline /> or  <EquationSnippet equation="expected" inline />) and <EquationSnippet equation="\Delta_R" inline /> is the difference between them.
        </div>
    </p>
    ...
    <EquationSnippet equation="\pi_{new}(a\|s) = \pi_{old}(a\|s) \cdot (1.0 + \alpha \cdot \Delta_R)" inline={false} />

    <div class="equation-note">
        – where <EquationSnippet equation="\pi_{new}(a\|s)" inline /> is read as "new policy value for action <EquationSnippet equation="s" inline /> in state <EquationSnippet equation="s" inline />.
    </div>
    ...
    <p style="padding-left: 8px">
        "Set the new policy value for action <EquationSnippet equation="a" inline /> in state <EquationSnippet equation="s" inline /> to the old policy value for action
        <EquationSnippet equation="a" inline /> in state <EquationSnippet equation="s" inline /> multiplied by the sum of one plus the product of the learning rate <EquationSnippet equation="\alpha" inline /> and the reward delta <EquationSnippet equation="\Delta_R" inline />."
    </p>
...
<LightboxImage image="/src/assets/images/hiivelabs.com/blog/2024-10-30-ml-experiments-pt-1/raw_policy.png" alt="Raw Policy" />
...
<ContentWarning warning="CONTENT WARNING: EXCESSIVE FUZZY DETAILS — CLICK TO SHOW AT YOUR OWN RISK!">
    ...
    * Policy: Adjust heating or cooling based on the fuzzy membership degrees.
    ...
    <div class="data-table">
        ...
        | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
        | `Cold`    | <EquationSnippet equation="\mu_{\text{Cold}}(T) = \begin{cases} 1, & T \leq 15 \\ \dfrac{20 - T}{5}, & 15 < T < 20 \\ 0, & T \geq 20 \end{cases}" inline style="color: rgb(var(--text-main) / var(--tw-text-opacity))" />                                             |
        | `Ideal`   | <EquationSnippet equation="\mu_{\text{Ideal}}(T) = \begin{cases} 0, & T \leq 17 \text{ or } T \geq 23 \\ \dfrac{T - 17}{3}, & 17 < T < 20 \\ \dfrac{23 - T}{3}, & 20 \leq T < 23 \end{cases}" inline style="color: rgb(var(--text-main) / var(--tw-text-opacity))" /> |
        | `Hot`     | <EquationSnippet equation="\mu_{\text{Hot}}(T) = \begin{cases} 0, & T \leq 25 \\ \dfrac{T - 25}{5}, & 25 < T < 30 \\ 1, & T \geq 30 \end{cases}" inline style="color: rgb(var(--text-main) / var(--tw-text-opacity))" />                                              |

        <br />

        These membership functions can be visualized as in the following plot:

        <LightboxImage image="/src/assets/images/hiivelabs.com/blog/2024-10-30-ml-experiments-pt-1/fuzzy_membership_temp.png" alt="Membership functions for Temperature fuzzy set" />

        Let’s consider a temperature of 18°C and calculate our membership degrees.

        * `Cold`: Since <EquationSnippet equation="15 < 18 < 20" inline />:

        <div class="ml-16 lg:ml-32">
            <EquationSnippet equation="\mu_{\text{Cold}}(18) = \dfrac{20 - 18}{5} = \dfrac{2}{5} = 0.4$$" inline style="color: rgb(var(--text-main) / var(--tw-text-opacity))" />
        </div>
        * `Ideal`: Since <EquationSnippet equation="17 < 18 < 20" inline />:

        <div class="ml-16 lg:ml-32">
            <EquationSnippet equation="\mu_{\text{Ideal}}(18) = \dfrac{18 - 17}{3} = \dfrac{1}{3} \approx 0.333" inline style="color: rgb(var(--text-main) / var(--tw-text-opacity))" />
        </div>
        * `Hot`: Since <EquationSnippet equation="18 \leq 25" inline />:

        <div class="ml-16 lg:ml-32">
            <EquationSnippet equation="\mu_{\text{Hot}}(18) = 0" inline style="color: rgb(var(--text-main) / var(--tw-text-opacity))" />
        </div>
        ...
        * Not `Hot`: Membership degree of `0`.
        ...
        <LightboxImage image="/src/assets/images/hiivelabs.com/blog/2024-10-30-ml-experiments-pt-1/2d_fuzzy_patches.png" alt="Membership sets for an autonomous vehicle navigating based on speed and distance to an obstacle." />
