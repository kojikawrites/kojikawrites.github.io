...
- The final `IF` ensures that cells remain in the `on` state if they had exactly two neighbors in the previous step.

These checks rely on the previous state of the grid rather than any updates already made in the current iteration.
...
Two well-known variations for generating mazes are Maze (`B3/S12345`) and Mazectric (`B3/S1234`). The main distinction
is that Mazectric tends to produce longer corridors. Only the relevant script excerpts are shown here.
