...
Update <FormattedDate date="1/21/2025" />: I did a few more tests of the algorithm after I posted the above to see how
well it handled an "extreme" case. The chosen subject was the well known Yoshi sprite from Super Mario World on the SNES.
It's a difficult case because it contains a lot of detail packed into its 32×32 pixel grid.

The following images show, in order, the input 32×32 Yoshi sprite, 16×16 nearest neighbor downscaled sprite,
16×16 bilinear downscaled sprite, and finally the 16×16 adaptive detail downscaled sprite.
...
Even though (in my opinion) the adaptive downscaling still produces the best results, the output is still not great,
and shows that with high detail source images, the algorithm is limited in what it can pack into the smaller space.
