...
choice not to do it this way. Instead, I chose to partition the maze in "board game" fashion, as shown in the following
image:
...
Moving across rectangular tiles (shaded darker gray) incurs no cost. Only the square tiles (in lighter gray) incur a
...
the movement cost being incurred for each vector traversed. The second shows the paths as spaced-out squares with the
center of each square corresponding to the endpoint of each vector in the first image.
...
also in the parent project. I'm looking forward to being able to announce it officially, but I'll need to make significantly
